...
// ── 移植说明 ─────────────────────────────────────────
// 派生自 MAXeaglet/commandcode-proxy(MIT)的 test/helpers.mjs;
// 仅改 startProxy:参考项目复制单文件 proxy.mjs,本项目入口是
// server.mjs + src/,故改为复制这两个路径并 spawn server.mjs,
// 并以 CCP_DATA_DIR 指向临时工作目录(复刻 data/config.json 部署形态)。
import http from 'node:http';
import { spawn } from 'node:child_process';
import { setTimeout as sleep } from 'node:timers/promises';
import { mkdtempSync, copyFileSync, cpSync, existsSync, rmSync } from 'node:fs';
...
export async function startMockUpstream(opts = {}) {
  ...
  const server = http.createServer((req, res) => {
    ...
    req.on('end', async () => {
      ...
      // onRequest 返回 false = 调用方完全接管该响应(挂起、断连等场景),跳过默认应答
      if (opts.onRequest) {
        const handled = await opts.onRequest(req, res, seen[seen.length - 1]);
...
/** 在临时 cwd 中启动代理（复刻真实部署：server.mjs + src/ 与数据目录）。 */
export async function startProxy({ upstreamPort, env = {}, cwd } = {}) {
  ...
  copyFileSync(join(REPO, 'server.mjs'), join(workdir, 'server.mjs'));
  cpSync(join(REPO, 'src'), join(workdir, 'src'), { recursive: true });
  if (!existsSync(join(workdir, 'config.json'))) {
    copyFileSync(join(REPO, 'config.default.json'), join(workdir, 'config.json'));
  }
  const child = spawn(process.execPath, ['server.mjs'], {
    cwd: workdir,
    env: { ...process.env, PORT: String(port), HOST: '127.0.0.1',
      CCP_DATA_DIR: workdir,
  ...
  return {
    port, base, child, dir: workdir, logs: () => logs.join(''),
